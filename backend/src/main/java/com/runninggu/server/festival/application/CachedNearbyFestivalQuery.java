...
/**
 * 대회별 인근 축제 **기본 목록**을 하루 캐시한다. (SPEC §8.3 · API 명세 §3-5)
 *
 * 공식 페이지(`officialUrl`)는 여기서 붙이지 않는다 — 이 결과가 하루 캐시라, 안에서 붙이면
 * 일시적 조회 실패가 하루 동안 null 로 굳는다(#352 리뷰). {@link NearbyFestivalService} 가
 * 캐시 바깥·반환 직전에 붙인다.
 */
@Service
@Transactional(readOnly = true)
public class CachedNearbyFestivalQuery {
    ...
    public CachedNearbyFestivalQuery(
    ...
    /** 대회일 ±14일·반경 40km 축제를 거리순 여섯 건까지 반환한다. `officialUrl` 은 null 이다. (SPEC §8.3) */
    @Cacheable(
