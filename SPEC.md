...
| C1 | 커뮤니티 — 동선 공유 | S9 | 백엔드 DB | P1 — **공모전 핵심 MVP 이후 착수** 🔒확정 (AP-15) |
| C2 | 커뮤니티 — 지역 기반 유저 러닝코스 공유 | S9 | 백엔드 DB | P1 — **MVP 이후** 🔒확정 (AP-15) |
...
- **앱 시작**: 스플래시에서 DataStore 세션 확인 → 세션 있으면 `home`, 없으면 `login`. **게스트 둘러보기 허용** 🔒확정 — 탐색(S1·S2·S3·S8)은 비로그인 가능, 저장(보관함 포함)·커뮤니티는 로그인 요구.
...
- 메시지: "보관함에 저장했어요" 🆕회의 · "동선을 공유했어요" · "동선을 못 불러왔어요. 다시 시도해 주세요" · "인증 메일을 보냈어요" · "가입이 완료됐어요" · "재설정 링크를 이메일로 보냈어요" · "커뮤니티에 공유했어요" · "보관함에 담았어요" 🔧정책
...
- 구글·네이버 버튼: P2 🆕회의.
- **게스트 둘러보기** 🔒확정: 탐색(S1·S2·S3·S8)은 게스트 허용, 저장(보관함 포함)·커뮤니티는 로그인 요구 — 진입 시 로그인 화면으로 유도. 로그인 화면에 "둘러보기" 진입점 제공 🔧정책.
...
### 4.12 S9 커뮤니티 🆕회의 (백엔드 필수 · **공모전 핵심 MVP 이후 착수** 🔒확정)
...
- 좋아요·댓글·신고: [미결-4]. 신고·관리자 삭제는 권장(NFR-13). 게스트 읽기 전용 🔒확정(게스트 범위 결정 준용 — 등록·담기는 로그인 필요).
- MVP 빌드에서의 탭 처리 🔧정책: 커뮤니티 탭은 유지하되 "준비 중이에요" 안내 화면 노출 (또는 MVP 동안 탭 숨김 — 팀 선택).
...
### 6.5 회원·커뮤니티 계약 🆕회의 (**Spring Boot + PostgreSQL** 🔒확정 — 구현 시 테이블 상세화)
...
회원(U1~U3)·커뮤니티(C1·C2)·보관함 동기화(B1)·조회수(H1)·**외부 API 프록시(REST 키 격리 🔒확정)** 를 서버가 담당한다.

- 스택: **Spring Boot + PostgreSQL** + Spring Mail(SMTP — 가입 인증 코드·재설정 링크) + Spring Security(세션은 JWT 액세스+리프레시 🔧정책).
- 서버 역할 4가지: ① 인증·회원 ② 보관함·커뮤니티 영속화 ③ 조회수 집계(H1) ④ **KTO·카카오 REST 프록시** — POI·축제·지오코딩·걷기 스팟·이동시간. 프록시에는 서버 캐싱·레이트리밋을 둔다 🔧정책(쿼터 §9.5).
- **API 계약은 OpenAPI 문서로 확정** — springdoc-openapi(코드 기반 Swagger 자동 생성) 권장, 방식 확답은 [미결-13]. §9.3 초안이 시드.

### 9.3 서버 API 초안 🔧정책 (OpenAPI 문서화 대상 — [미결-13])
...
| AP-07 | **백엔드 구축 — Spring Boot + PostgreSQL** 🔒확정: 인증(U1~U3, 재설정 링크)·보관함·조회수·**외부 API 프록시** + OpenAPI 문서(springdoc 권장, [미결-13]) | §9.2·9.3 |
...
| ID | 항목 |
|---|---|
| AP-15 | S9 커뮤니티 — **공모전 핵심 MVP 완료 후 착수** 🔒확정 (동선 공유 피드 + 코스 공유, 범위 [미결-4] 확정 후. MVP 빌드는 '준비 중' 안내 🔧정책) |
...
| 결정-5 | 커뮤니티는 공모전 핵심 MVP 이후 배치 | §1.2 · §4.12 · AP-15 |
| 결정-6 | 비밀번호 재설정: **재설정 링크** 방식 *(구 미결-7 — 07-16 회의안 '임시 비밀번호' 대체)* | §4.3 · 부록 C-3 |
| 결정-7 | SPEC 안드로이드 기준 갱신 | v3 전체 |
| 결정-8 | API 계약을 OpenAPI 문서로 확정 — **문서화 방식만 확답 대기** → [미결-13] | §9.2 · §9.3 |
...
| 4 | 커뮤니티 **범위** (시점은 'MVP 이후'로 확정 — 결정-5) | 코스 등록 방식(왕복 경로 공유/지도 그리기/GPX), 좋아요·댓글, 신고 절차 |
...
| 12 | **minSdk·지원 범위** | 카카오맵 SDK v2 최소 요구(공식 문서 확인) 이상에서 확정. 제안: minSdk 26+ |
| 13 | **API 문서화 방식** (결정-8 후속) | 권장: 백엔드에 **springdoc-openapi** 적용 — 컨트롤러 코드에서 Swagger UI 자동 생성, 앱 팀은 그 문서를 보고 Retrofit DTO 작성 (공수 최소). 대안: `openapi.yaml` 선작성(문서 우선) 후 코드 생성. §9.3 초안이 시드 |

> 6·7·10·11번은 12.1의 결정-4·6·3·2로 해소되어 목록에서 제외했다 (번호는 문서 내 참조 보존을 위해 재사용하지 않음).
