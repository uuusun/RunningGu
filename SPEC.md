...
> 📌 마감일이 미래라는 것만으로는 접수중이라고 단정할 수 없다 — 아직 접수 시작 전일 수 있다. API 명세 §3-1 목록 응답에 `applyStart` 가 없어 실제로 생기는 상황이며, 이때는 서버가 조회 시점에 파생한 값을 그대로 믿는다(이슈 #44). 서버는 DB 에서 항상 양쪽 날짜를 보므로 서버 파생 동작은 이 규칙과 어긋나지 않는다.
