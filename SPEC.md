...
- 가입 또는 비밀번호 재설정이 성공하면 해당 `emailVerification` 행을 같은 트랜잭션에서 즉시 삭제한다. 사용하지 않은 만료 행과 기존 소비 완료 행은 애플리케이션 시작 시와 이후 매시간 멱등 정리한다. 정상 운영 시 만료 후 1시간 이내, 운영 상한은 24시간 이내 삭제이며 회원 탈퇴 시 정규화 이메일의 가입·재설정 기록을 즉시 삭제한다. 만료 여부는 정리 여부가 아니라 요청 시각으로 판정한다(결정-57).
...
| NFR-20 🔒확정 | **인증 데이터 보존·복구** — 인증 성공 기록은 가입·재설정 완료 트랜잭션에서 삭제하고, 만료 인증·Refresh Token 행은 시작 시·매시간 정리해 정상 1시간·최대 24시간 안에 삭제한다. 암호화 일일 백업은 7일 보관 후 자동 삭제하며 운영책임자만 접근한다. 복원 시 탈퇴 삭제와 만료 정리를 재적용·검증하기 전에는 외부에 공개하지 않는다(결정-57) |
...
| 결정-57 | 이메일 인증·재설정 완료 행은 성공 트랜잭션에서 삭제하고 만료 인증·Refresh Token 행은 시작 시·매시간 정리한다. 정상 1시간·최대 24시간 보존 상한, `LOGIN_IDENTITY → EMAIL_VERIFICATION → REFRESH_TOKEN` 잠금 순서, Refresh 14일·재사용 시 같은 family 전체 revoke를 적용한다. 암호화 일일 백업은 7일 보관하고 복원 시 탈퇴 삭제·만료 정리를 재적용·검증하기 전 외부에 공개하지 않는다 | 만료 판정이 정리 시각에 따라 달라지거나 탈퇴·재설정 경합으로 데이터가 되살아나는 일을 막고, 토큰 재사용 탐지와 개인정보 최소 보유를 함께 지킨다(이슈 #227 · 2026-08-31 팀 결정) | §4.2 · §6.5 · §9.3 · NFR-20 |
