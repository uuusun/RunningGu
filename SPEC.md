...
- [카카오로 시작하기] 📱전환: **카카오 로그인 Android SDK** — 카카오톡 설치 시 앱 전환 간편로그인, 미설치 시 카카오계정 로그인. 발급 토큰을 백엔드로 전달·검증(§9.3) → 기존 KAKAO 가입 계정이면 로그인, 미가입이면 A2 카카오 가입 플로우. 카카오 사용자 정보 응답의 닉네임·이메일은 동의 상태에 따라 둘 다 없을 수 있으므로 가입 화면 초기값은 nullable이다.
...
- **백엔드(서버 환경변수)** 🔒확정: `KTO_SERVICE_KEY`(+ENC) · `KAKAO_REST_KEY` · SMTP 자격증명 · JWT 서명 키 · DB 접속 정보 — 전부 서버 전용, 앱 배포물·저장소 포함 금지.
