...
> | ② | **팀 추가 결정 (2026-07-30, 2026-08-19 계정 정책 개정, §12.2)** — 4탭·마이 정보수정·사용자당 단일 가입 수단·canonical 대회·시스템 RACE·공통 API 계약·두루누비 동기화·UTF-8 CI | 전부 반영 |
...
- 계정 모델 🔒(결정-22 개정): 사용자는 가입 시 `EMAIL` 또는 `KAKAO` 중 하나를 선택하고, 한 `USER`는 정확히 하나의 `LOGIN_IDENTITY`만 가진다. P0에서는 로그인 수단 연결·추가·전환을 지원하지 않으며 서로 다른 provider 계정을 이메일만으로 자동 병합하지 않는다.
- 구글·네이버 버튼: P2 🆕회의.
...
상단에 프로필 요약(닉네임·대표 이메일·가입 로그인 방식)과 [정보 수정]을 두고, 아래에 세그먼트 3개 **[동선] | [러닝코스] | [찜한 대회]**를 유지한다.
...
- 대표 이메일은 EMAIL 가입자면 정규화된 `providerSubject`, KAKAO 가입자면 nullable `emailSnapshot`에서 파생한다. USER에 이메일을 중복 저장하지 않는다.
- EMAIL 수단은 `passwordHash`와 `emailVerifiedAt`이 필수이고 `emailSnapshot`은 null이다. KAKAO 수단은 비밀번호·자체 이메일 인증 시각이 null이며 카카오 회원번호를 로그인 식별자로 사용한다.
...
| 결정-22(08-19 개정) | EMAIL/KAKAO는 `LOGIN_IDENTITY`로 분리하되 **한 USER는 가입 시 선택한 수단 하나만 보유**한다. P0 연결·추가·전환과 동일 이메일 자동 병합은 지원하지 않는다 | 제품에 계정 연결 기능이 없으므로 USER와 로그인 수단을 1:1로 고정하고 불필요한 연결·해제 계약을 제거한다 | §4.1 · §4.13 · §6.5 |
...
**P0 제품 미결은 `TBD-DB-02` 저장 코스 attribution, `TBD-DB-04` 저장 동선 표시값 snapshot/파생, `TBD-DB-05` 주간 대회 snapshot 누락 처리다.** 팀 논의용 GitHub 이슈에서 결정한다. `TBD-DB-01`은 좌표 정밀도와 고도 배열 PostgreSQL 타입만 남았고, D-21(saved/ran 통합 정렬·페이징)은 결정-36에 따라 GPS P1 착수 시 결정한다. 구현 중 새 결정이 필요하면 PR 본문에서 논의하고, 확정 시 §12에 추가한다.
