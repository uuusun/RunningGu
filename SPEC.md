...
- **시각은 블록의 것이고, 시각을 고치면 자리가 따라온다** 🔧정책(#319 · 2026-09-10).
  - 순서를 바꿔도 **시각은 블록을 따라간다.** 자리를 옮긴 뒤 시각이 안 맞으면 시각을
    직접 고쳐 맞춘다 — 드래그가 시각을 자동으로 다시 매기는 것은 §5-8·§5-10 계약을
    함께 바꿔야 해서 미뤘다(#335)
  - **저장 전 결과의 시각을 직접 고칠 수 있다.** 왼쪽 시각 열을 누르면 숫자로 입력한다(`HH:mm`). 고치면
    **그 시각에 맞는 자리로 옮겨진다** — 시각이 정답이고 순서가 따른다. 대회 블록이 나눈
    구간 안에서만 정렬하므로 RACE 는 제자리를 지키고, RACE 의 시각은 못 고친다. 저장 후
    시각 편집은 `PATCH startTime`과 순서 변경을 한 트랜잭션으로 묶는 계약을 #335에서
    정하기 전까지 제공하지 않는다 — 두 요청을 이어 보내 부분 성공 상태를 만들지 않는다
  - **새 블록의 시각**은 넣는 자리에서 정한다. 맨 끝이면 마지막 시각 한 시간 뒤,
    사이면 앞뒤의 가운데, 첫 줄이면 뒤 블록보다 한 시간 앞이다. 하루가 비어 있을 때만
    §5-7 계약 기본값(`13:00`)을 쓴다 — 이미 17시까지 찬 하루에 13시가 끼어들면 안 된다
- **장소는 검색으로도 추가한다** 🔧정책(#319). 후보 시트에 검색어 입력을 둔다. 서버
  `GET /api/places` 가 `query` 를 받으므로 계약 변경은 없다. 검색어가 두 자 미만이면
  기준점 주변 추천으로 돌아간다
