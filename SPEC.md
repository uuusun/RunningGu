...
- **nginx 파일 로그** 🔒확정(결정-61): 접속·오류 로그는 매일 회전하고 14일 보관 후 정기 삭제한다. 앱 host의 HTTP·HTTPS 접속 로그에는 URL 쿼리 문자열을 기록하지 않고, 회전본은 압축하며 새 파일은 `0640 www-data:adm`으로 만든다.
...
| 결정-61 | nginx 접속·오류 파일 로그는 매일 회전하고 회전본 14개와 `maxage 14`를 적용해 **14일 보관 후 정기 삭제**한다. 회전본은 압축하고 새 파일은 `0640 www-data:adm`으로 만들며, 앱 host의 HTTP·HTTPS 접속 로그에는 URL 쿼리 문자열을 기록하지 않는다 | 보안·장애 확인에 필요한 최소 기간을 명시하고 개인정보처리방침과 실제 자동 삭제 설정을 일치시킨다. 로그인·중복 확인 제한용 IP 메모리 캐시의 2분 만료와 nginx 파일 로그 보관은 서로 다른 정책이다(2026-09-07 운영책임자 결정) | §9.4 · NFR-9 · NFR-19 |
