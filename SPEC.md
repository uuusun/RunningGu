...
| 결정-63 | 동선 화면에서는 블록 시각을 표시하거나 편집하지 않고 일정 순서는 `orderNo`와 드래그로만 관리한다. 서버 `startTime` 필드와 DB는 기존 계약 호환을 위해 유지하며, 앱의 새 블록은 기본값 `13:00`을 보내되 노출하지 않는다 | 시각 변경과 순서 재정렬을 두 요청으로 처리할 때 생기는 부분 성공을 없애고, API·DB 마이그레이션 없이 사용자 화면의 일정 순서를 하나의 기준으로 단순화한다(이슈 #335 · 2026-09-11 서버 담당 결정) | §4.10 · §5.7~§5.10 |
