...
4. **축제·지역 관광 추천** — 사용자 위치·위치 권한을 사용하지 않고 조회 월의 전국 진행 중·임박 축제 캐러셀을 보여준다 🔒확정. 서버가 KTO `searchFestival2`를 호출·캐시하고 앱은 우리 서버만 호출한다. 출처 "한국관광공사"(NFR-7). 카드를 펼치면 [공식 페이지 ↗] — 서버가 `detailCommon2` `homepage`에서 뽑은 `officialUrl`을 Custom Tabs로 연다. 앱 안 축제 상세 화면은 만들지 않는다(결정-66).
...
- **인근 축제 (M3)** 🧩목업: 백엔드 `GET /contests/{id}/festivals` 경유 🔒확정(§8.3) — 대회일 ±14일 → 반경 40km → 거리순 6건. 카드: 이미지(`firstimage`, Coil 🔧정책) · 축제명 · "MM.DD~MM.DD · 대회장 {d.d}km" · [공식 페이지 ↗](`officialUrl` 있을 때만 · 결정-66). 로딩 "축제를 찾는 중…" / 빈 상태 "대회 기간에 열리는 인근 축제가 없어요." + "대회 전후 여행 동선은 그대로 짜드릴 수 있어요." 출처 "한국관광공사".
...
- **공식 페이지(`officialUrl`)** 🔒확정(결정-66): 홈·인근 축제 응답의 노출분(홈 `size`·인근 6건)에 한해 `detailCommon2`의 `homepage`에서 첫 http·https `href`를 꺼낸다. **contentId 별 1일 캐시.** 없거나 실패하면 그 항목만 null이고 목록은 살린다.
...
| **축제 공식 페이지 (`detailCommon2`)** | 홈 노출분(≤20)·인근 6건만, **contentId 별 1일 캐시** — 하루 신규 축제 수만큼만 나간다 | 여유(결정-66) |
...
| 결정-66 | 홈 축제 카드와 대회 인근 축제에 [공식 페이지 ↗] 를 둔다. 서버가 `GET /api/festivals`·`GET /api/contests/{id}/festivals` 응답에 nullable `officialUrl`을 추가하고, 값은 KTO `detailCommon2` `homepage`의 첫 http·https `href`다. 노출분만 조회하고 contentId 별 1일 캐시한다. 앱 안에 축제 상세 화면은 만들지 않으며(D-05 유지) null이면 버튼을 숨긴다 | 원스토어 출시 직후 "축제를 눌러도 아무것도 안 뜬다"는 피드백. 상세 화면을 새로 만들지 않고도 사용자가 다음 행동(공식 페이지)으로 갈 수 있게 하되, 축제당 한 번 더 나가는 KTO 호출이 개발계정 쿼터(§9.5)를 넘지 않게 막는다(2026-09-14 · 서버 담당 위임으로 앱 코어가 서버·앱 함께 반영) | §4.4 · §4.6 · §8.3 · §9.5 · API 명세 §3-5·§4-1 |
