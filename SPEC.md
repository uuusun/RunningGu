...
1. **약관·개인정보 동의와 연령 확인** — 필수 2종(이용약관/개인정보 수집·이용) + 선택(마케팅) 🔧정책. 전체 동의 제공. **만 14세 이상 확인은 전체 동의 밖의 별도 필수 항목**이며 전체 동의로 자동 선택하지 않는다. 생년월일·실제 나이는 수집하지 않고 별도 연령 컬럼도 두지 않는다. 이메일·카카오 가입 요청은 최상위 `ageOver14`를 필수로 받으며 누락은 `400 VALIDATION_FAILED`, `false`는 `400 AGE_REQUIREMENT_NOT_MET`으로 가입을 거부한다. 가입 완료 뒤에는 TOS 1.1 동의 버전·시각이 연령 확인 근거가 된다(결정-58, 이슈 #228).
   현재 활성 버전은 `TOS=1.1`, `PRIVACY=1.2`, `MARKETING=1.1`이고 서버가 세 항목을 같은 변경 시각으로 이력 저장한다(NFR-12, 결정-52(09-11 개정), 이슈 #111). 셋의 번호가 서로 다른 것은 `PRIVACY 1.1`이 기기 위치 제거(#215)로 사실과 어긋나 한 번도 켜지지 않은 채 `1.2`로 대체됐기 때문이다. 앱 `AgreementDoc`과 서버 `runninggu.auth.agreements.*-version`은 **같은 PR에서** 함께 올린다(#278). 소스 정합과 운영 서버 전환·앱 공개는 다른 시점이며, 운영 전환은 출시 때 수행한다(`docs/agreements/README.md`).
...
| 결정-52(09-11 개정) | 활성 가입 약관 버전은 `TOS=1.1`, `PRIVACY=1.2`, `MARKETING=1.1`이며 가입 시 선택 동의를 포함한 세 항목을 같은 시각으로 append-only 저장한다. 앱은 버전을 보내지 않고 동의 boolean만 전송한다 | 앱 카피와 서버 이력 버전이 독립적으로 어긋나지 않게 하며, 이후 버전 변경은 앱·서버 계약 변경으로 함께 리뷰한다(이슈 #111) | §4.2 · §6.5 · NFR-12 |
