...
- **시각은 자리에 붙고, 시각을 고치면 자리가 따라온다** 🔧정책(#319 · 2026-09-10).
  - 순서를 바꾸면 각 줄의 시각은 **원래 그 자리의 것**이다. 예전에는 시각이 블록을 따라와
    `17:00` 일정을 위로 올려도 `17:00` 이었다 — 사용자가 읽는 것은 그 자리의 시각이다
  - **시각을 직접 고칠 수 있다.** 왼쪽 시각 열을 누르면 숫자로 입력한다(`HH:mm`). 고치면
    **그 시각에 맞는 자리로 옮겨진다** — 시각이 정답이고 순서가 따른다. 대회 블록이 나눈
    구간 안에서만 정렬하므로 RACE 는 제자리를 지키고, RACE 의 시각은 못 고친다
...
- **장소는 검색으로도 추가한다** 🔧정책(#319). 후보 시트에 검색어 입력을 둔다. 서버
  ...
  기준점 주변 추천으로 돌아간다 | 2026-07-30 검증에서 목업 외 구현이 필요하다고 확정한 항목 |
