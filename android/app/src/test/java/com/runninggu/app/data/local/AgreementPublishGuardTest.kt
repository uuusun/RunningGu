...
class AgreementPublishGuardTest {
     ...
     *
     * 활성이 `v1.0` 이던 동안에는 위 테스트가 v1.2 를 보지 않아서, 이 구분이 깨져도
     * 활성 버전을 올리는 PR 에서야 빨간불이 됐다. 지금은 `PRIVACY 1.2` 가 활성이라 위
     * 테스트도 이 줄을 지나간다 — **그래도 여기를 남긴다.** 위 테스트는 "지금 문안에
     * 표시가 없다" 를 보고, 이 테스트는 "가르는 규칙이 맞다" 를 본다. 다음 문안에서
     * 주소가 바뀌어도 규칙이 무너지지 않게 하는 것은 이쪽이다.
     */
    @Test
