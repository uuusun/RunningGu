...
class DomainRulesTest {
    ...
    @Test
    fun `종목 추출은 세 단계의 합집합이다`() {
        // 이 테스트는 원래 "플래그가 거리보다 우선한다" 였다. §5.4 가 합집합으로 개정돼
        // 기대값을 바꿨다 — 플래그는 이제 거리를 가리지 않는다 (이슈 #61)
        val flagAndDistance = eventsFromRace(
            flags = EventFlags(half = true),
            distancesKm = listOf(42.0),
        )
        assertEquals(listOf(EventType.FULL, EventType.HALF), flagAndDistance)
    ...
    @Test
    fun `무주 풀코스는 종목 넷이 다 나온다`() {
        // has_full 만 켜져 있고 나머지는 거리에만 있다. 우선순위였다면 풀 하나로 줄었다
        val got = eventsFromRace(
            flags = EventFlags(full = true),
            distancesKm = listOf(42.195, 24.0, 12.0, 8.0, 4.0),
        )

        assertEquals(
            listOf(EventType.FULL, EventType.HALF, EventType.TEN_K, EventType.FIVE_K),
            got,
        )
    }

    @Test
    fun `토큰 단계는 모르는 말을 5K 로 넣지 않는다`() {
        // stdEvent 는 "그 외 5K" 지만 그건 종목 하나를 표준화할 때의 규칙이다.
        // 목록을 만들 때 모르는 토큰을 넣으면 없는 종목이 생긴다 (§5.4 ③)
        assertEquals(emptyList<EventType>(), eventsFromRace(eventTypes = listOf("걷기", "15km")))
        assertNull(stdEventToken("걷기"))

        // 오매칭 방지 — 110km 의 10k, 15km·4.5km 의 5k
        assertNull(stdEventToken("110km"))
        assertNull(stdEventToken("15km"))
        assertNull(stdEventToken("4.5km"))
        assertEquals(EventType.TEN_K, stdEventToken("10km"))
        assertEquals(EventType.FIVE_K, stdEventToken("5km"))
    }

    @Test
    fun `0 과 음수 거리는 종목으로 세지 않는다`() {
        // 결측을 0 으로 채운 행이 5K 로 둔갑하면 안 된다
        assertEquals(emptyList<EventType>(), eventsFromRace(distancesKm = listOf(0.0, -5.0)))
    }
