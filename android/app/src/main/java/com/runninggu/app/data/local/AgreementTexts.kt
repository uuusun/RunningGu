 ...
 * `tos-version` · `privacy-version` · `marketing-version` 을 각각 관리한다. 지금 활성은
 * TOS `v1.1` · PRIVACY `v1.2` · MARKETING `v1.1` 로 **셋이 서로 다르다** — PRIVACY 만
 * `v1.1` 을 건너뛰고 `v1.2` 로 갔다(기기 위치 기능이 빠져서다 · #215). 하나로 묶었다면
 * 이 순간 있지도 않은 `v1.2/tos.md`를 읽어 **두 문안이 안 열리고 버전 표시까지
 * 틀렸다**(#191 리뷰).
 */
object AgreementTexts {
    ...
    /** `docs/agreements/v1.1/tos.md` 가 assets 에서는 `v1.1/tos.md` 다. */
 ...
 *  **같아야 한다.** 셋을 하나로 묶지 않는 이유는 서버가 따로 관리하고 실제로 버전이
 *  갈리기 때문이다 — 지금도 TOS `1.1` · PRIVACY `1.2` · MARKETING `1.1` 이다. 하나로
 *  묶으면 한 문안을 올릴 때 나머지 파일 경로가 깨진다(#191 리뷰). 올릴 때는 서버 설정과
 *  **같은 PR 에서** 바꾼다(D-32).
 */
enum class AgreementDoc(
    ...
    TOS("이용약관", "tos.md", required = true, version = "1.1"),
    PRIVACY("개인정보 수집·이용", "privacy.md", required = true, version = "1.2"),
    MARKETING("마케팅 정보 수신", "marketing.md", required = false, version = "1.1"),
    ;
