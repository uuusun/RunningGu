...
private class Picker(
     ...
     * 첫 카테고리를 쓴다. 전부 소진되면 관광지로 떨어진다.
     *
     * ## 이것은 SPEC §5.6-5 의 **현재 규칙이 아니다** (결정-72 · #377)
     *
     * 취향 카테고리 순환·맛집 제외·회복 골격의 취향 자리는 **서버 생성기에만 반영했다.**
     * 동선 생성은 서버 단일 주체이고(결정-41) 이 엔진은 어느 화면에도 연결돼 있지 않아서,
     * 규칙을 두 벌로 유지하면 어느 쪽이 기준인지 흐려진다. 규칙의 기준은 `SPEC.md` §5.6 과
     * `backend` 의 `ItineraryGenerator` 다.
     *
     * 이 엔진을 되살릴 일이 생기면 그때 §5.6 을 다시 보고 함께 옮긴다.
     */
