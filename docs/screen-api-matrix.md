...
|---|---|---|---|---|
| 약관 동의 | 로컬 | tos, privacy, marketing | 필수 2종 동의 시 다음 활성화. 활성 카피 버전 `TOS=1.1 · PRIVACY=1.2 · MARKETING=1.1` | 가입 완료 전 로컬, 가입 시 서버가 버전 포함 3행 저장 |
...
| D-32 / SPEC 결정-52(09-11 개정) | 현재 가입 화면 약관 카피와 서버 활성 버전은 `TOS=1.1 · PRIVACY=1.2 · MARKETING=1.1`으로 맞춘다. 앱은 boolean만 보내고 버전 변경은 앱·서버가 같은 PR에서 함께 올린다(#278). 소스 정합과 운영 서버 전환·앱 공개는 다른 시점이다(이슈 #111 · #265) |
