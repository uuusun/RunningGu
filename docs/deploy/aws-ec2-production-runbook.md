...
key는 2026-09-10 운영 결정에 따라 같은 계정의 staging 값과 공유하며 호출 쿼터도 합산 관리한다.
...
KTO service key는 2026-09-10 결정에 따라 staging과 공유한다. 값은 명령 인자·shell history·
Git·CI·문서에 남기지 않는다. 두 env 파일의 `DB_PASSWORD`만 같은 값을 사용한다.
