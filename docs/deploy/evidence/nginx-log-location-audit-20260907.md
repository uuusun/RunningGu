...
- #293은 공모전 심사 계정·표본 준비를 추적한다. 민지의 현재 요청은 staging에서 로그인·표본 생성을 시험할 **런닝구 앱 회원**이다.
- 새 이메일 서비스 계정이나 원스토어 개발자 계정을 만들라는 뜻이 아니다. 메일 수신·인증 가능한 주소를 앱 로그인 ID로 사용한다.
- 개인 데이터가 없는 기존 전용 테스트 회원이 목적에 맞으면 재사용 여부를 검토할 수 있다. 기존 회원 존재를 확인하지 않고 새 계정을 반드시 만들어야 한다고 단정하지 않는다.
- 최종 공모전 심사는 공개 앱이 연결된 운영 환경에서 접근 가능해야 한다. staging 시험 회원이 존재한다는 사실만으로 최종 심사 계정 준비가 완료되지 않는다.
- 원스토어 심사팀에 제공할 로그인 안내·계정은 실제 검증 제출 요구와 구분해서 확인한다. 두 용도의 계정을 무조건 각각 새로 생성한다는 결정은 하지 않았다.
