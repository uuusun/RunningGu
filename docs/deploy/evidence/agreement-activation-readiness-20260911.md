...
2. [완료] PR #278이 병합된 커밋 `42bfec46` 으로 최종 서명 APK를 만들고 A2에서 세 전문의
   버전·문안을 확인했다. `이용약관 버전 1.1` · `개인정보 수집·이용 버전 1.2` ·
   `마케팅 정보 수신 버전 1.1` 이 기기 화면에 표시되고, 초안 안내·내부 이슈 번호·검토 상태
   표시가 없다. APK 안 `assets/v1.1/tos.md` · `assets/v1.2/privacy.md` ·
   `assets/v1.1/marketing.md` 의 SHA-256 도 저장소 원본과 일치한다
   ([3회차 검증 기록](../../release/release-apk-verification.md)).
   같은 회차에서 운영 카카오 로컬 호출 실패를 발견했고 약관과는 무관하다
   ([기록](production-kakao-local-20260911.md)).
3. [완료 · 앞당겨 실행] 운영 서버를 `42bfec46` 으로 배포해 `TOS 1.1 · PRIVACY 1.2 ·
   MARKETING 1.1` 로 전환했다. 심사 통과 전이지만 **앱이 아직 공개되지 않아 현장에 구버전 앱이
   없고 심사에 올릴 APK 도 같은 문안을 번들**하므로, 먼저 맞추는 편이 동의 이력 정합에
   유리하다고 판단했다(2026-09-11 운영책임자 결정 ·
   [배포 기록](production-deploy-42bfec46-20260911.md)).
   같은 배포로 #334(결정-65)도 함께 반영됐다.

   > **전환 전 가입 1건의 동의 이력이 실제로 본 문안과 다르다.** 3회차 카카오 가입
   > (2026-09-11 09:21:55+00)이 서버 `1.0` 으로 기록됐는데 앱은 `1.1/1.2/1.1` 전문을 보여줬다.
   > 전환 뒤 신규 가입은 일치한다. 남은 1건은 해당 계정 탈퇴로 삭제한다.
   >
   > **서버는 앱이 보여준 버전을 검증하지 않는다.** 가입 요청에 버전 필드가 없고
   > `agreementProperties.tosVersion()` 을 그대로 찍으며, 그 값은 `application.yml` 하드코딩이라
   > 환경변수가 아니다. 배포 순서를 지키면 문제없지만 어긋나도 오류 없이 조용히 서버 값으로
   > 기록된다. 대조 계약을 둘지는 별도 판단이다.
