...
>
> **콘솔에 이미 있는 해시를 지우고 새로 넣지 말 것.** 여러 개를 등록할 수 있고, 지우면
> 그 해시를 쓰던 PC 가 그날부터 깨진다. **더하기만 한다.**
>
> **지도만의 이야기가 아니다.** 카카오 로그인도 같은 해시를 본다. 그런데 앱에는
> "카카오 로그인을 시작하지 못했어요" 하나로만 보여서 **화면으로는 미등록인지 다른
> 문제인지 못 가른다**(#216). logcat 의 인가 URL 에 있는 `ka=...origin/<키해시>` 를
> 위 명령 결과와 대조하면 바로 나온다.
...
- **오프라인** — P0 는 대회 초기본(`races.json`)과 대회·마감임박 Room 캐시로만 폴백하고, 응답의 `source` 를 UI 까지 전달한다. GPX 축약본과 마이 목록 캐시는 P1(SPEC 결정-66 · 결정-67).
