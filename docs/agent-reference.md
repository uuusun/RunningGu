...
| 문서 | 작성·갱신 | 무엇을 반영했나 |
|---|---|---|
| `docs/files/런닝구_API_명세서.md` | 2026-08-17 백엔드 기본 계약 갱신 | Java 21·Flyway·Testcontainers·Caffeine·JWT, S8는 기존 계약 |
| `SPEC.md` §9.2~9.3 | 2026-08-17 백엔드 기본 계약 갱신 | §9.2 최신, §9.3 S8는 기존 계약 |
| `docs/screen-api-matrix.md` | **2026-08-17 v1.0** | 화면 플로우·상태 전체, S8 계약은 이슈 #19 대기 |
...
화면 플로우와 상태 매핑은 최신화됐지만 S8 API 호출 방식은 아직 문서마다 다르다.
...
| `docs/screen-api-matrix.md` S8 | `/courses/near` 와 **따로 호출**. 이슈 #19 결정 대기 | 08-17 |
...
최신 화면 의도와 포팅 계획서는 "합쳐서 준다" 쪽이고, API 명세와 매트릭스의 S8 계약은 아직 따로 호출하는 형태다.
