...
#228 앱 후속 구현·테스트 범위다(D-35).

비밀번호는 BCrypt strength 10 단방향 해시(NFR-9). 출시 서버 소스의 활성 버전은 `TOS=1.1`, `PRIVACY=1.2`, `MARKETING=1.1`이며 선택 동의를 포함한 세 행을 같은 `changedAt`에 append-only 저장하고 앱은 버전을 보내지 않는다(NFR-12, 결정-52, 이슈 #111·#265). `ageOver14` 전용 DB 컬럼이나 `AgreementType`은 두지 않는다. TOS 1.1 동의 시각·버전이 만 14세 이상 제한 확인 기록 역할을 한다. 운영 적용은 앱·서버 소스 정합 뒤 최종 서명 APK 확인과 운영 서버 배포로 별도 검증한다(결정-58, 이슈 #228).
...
- 미인증 가입 코드는 `expires_at`, 인증됐지만 미가입인 행은 `verified_at + 30분`, 비밀번호
  ...
  만료된 요청을 성공시키지 않는다.
- 정리 대상은 SIGNUP 미인증 행은 `expires_at`, SIGNUP 인증 완료·미가입 행은
  `verified_at + 30분`, PASSWORD_RESET 행은 `expires_at`이 지난 기록과 기존 소비 완료
  행이다. 애플리케이션 시작 시 한 번, 이후 매시간 멱등 정리한다. 정상 운영 시 각 기준
  시각 후 1시간 이내, 대외 보유 상한은 24시간이다. 탈퇴 시 정규화 이메일의 가입·재설정
  기록을 탈퇴 트랜잭션에서 즉시 삭제한다.
...
```
빈 배열 = 목업 빈 상태("대회 기간에 열리는 인근 축제가 없어요") · `502` = 로딩 실패 상태 매핑.

`officialUrl`(nullable)은 축제 공식 페이지다. 서버가 반환 6건에 한해 KTO `detailCommon2`의 `homepage`에서
첫 `href`를 꺼내 준다(http·https 만, 그 외는 null). **contentId 별 1일 캐시**로 쿼터를 지킨다(§9.5).
6건은 **한꺼번에 조회하고 전체 3초 예산** 안에 끝낸다 — 차례로 부르면 KTO read timeout(2.5초)×건수라 앱
read timeout(15초)을 넘긴다. 예산 안에 안 온 것은 그 응답에서만 null 이고, 뒤늦게 끝난 조회는 캐시에 들어가
다음 요청이 쓴다. 이 보강은 **대회별 1일 목록 캐시 바깥·반환 직전**에 붙는다 — 안에서 붙이면 일시적 실패가
하루 동안 null 로 굳는다.
KTO에 등록되지 않았거나 조회에 실패하면 그 항목만 null이고 목록은 살린다 — 앱은 null이면
[공식 페이지 ↗] 버튼을 숨기고, 대회 `officialUrl`과 같은 기준(`openableWebUrl`)으로 Custom Tabs로 연다.
(2026-09-14 · 출시 후 "축제를 눌러도 아무것도 안 뜬다" 피드백) 출처 표기 "한국관광공사"는 클라 고정 문구(NFR-7). 대회 좌표가 없으면 외부 API를 호출하지 않고 `409 CONTEST_LOCATION_UNAVAILABLE`.
...
**수집·캐시 추적 메타데이터(`fetchedAt`·`cachedAt`·KTO 원천 구분)는 응답에 넣지 않는다.** 서버 내부 로그·모니터링에서만 관리한다 — 어느 화면도 그 값을 그리지 않기 때문이다(출처 표기는 위와 같이 클라 고정 문구다). 앱은 위 여덟 필드만 받는다.
...
응답은 `{"items": [...]}`이고 항목은 정확히 `{contentId, name, startDate, endDate, region, imageUrl, inProgress, officialUrl}`다. `inProgress` = start ≤ KST 오늘 ≤ end (진행중 배지).
...
`officialUrl`(nullable)은 축제 공식 페이지다. 서버가 **`size`로 자른 노출분에 한해** KTO `detailCommon2`의
`homepage`에서 첫 `href`를 꺼내 준다(http·https 만, 그 외는 null). 월 전체가 아니라 노출분만 부르고
**contentId 별 1일 캐시**를 두는 이유는 개발계정 쿼터(오퍼레이션당 일 1,000건 · SPEC §9.5)다 — 홈 목록
캐시(5분)에 묶으면 5분마다 최대 20건씩 다시 나간다. 노출분은 **한꺼번에 조회하고 전체 3초 예산** 안에
끝낸다(§3-5 와 같은 규칙) — 예산 안에 안 온 것은 그 응답에서만 null 이고 뒤늦게 끝난 조회는 캐시에 들어간다.
KTO에 등록되지 않았거나 조회에 실패하면 그 항목만 null이고 목록은 살린다.

**홈 축제 카드는 앱 안에 상세 화면이 없다** — 축제 상세로 이동하는 route는 만들지 않는다(결정 D-05).
대신 펼친 카드에 [공식 페이지 ↗] 를 두고 `officialUrl`을 Custom Tabs로 연다(2026-09-14 · 출시 후
"축제를 눌러도 아무것도 안 뜬다" 피드백). null이면 버튼을 숨긴다. 그 외 상세 조회용 키는 여전히 넣지 않는다.

**수집·캐시 추적 메타데이터(`fetchedAt`·`cachedAt`·KTO 원천 구분)는 응답에 넣지 않는다.** 서버 내부 로그·모니터링에서만 관리한다 — §3-5와 같은 정책이며, 어느 화면도 그 값을 그리지 않는다. 앱은 위 여덟 필드만 받는다. (오프라인 표시에 쓰는 `cachedAt`은 기기 Room 읽기 캐시의 것이라 이 응답 필드와 다른 이야기다.)
...
- **`themes` 배열의 순서는 SPEC §5.3 선언 순서다**(앱이 그 순서로 정렬해 보낸다 · 결정-72).
  ...
  보되 이번 일정에서 취향 자리로 이미 쓴 카테고리와 그 날 골격의 고정 카테고리는 뒤로 미룬다(SPEC §5.6-5).
...
- 생성용 FOOD는 최대 15건을 조회한 뒤 업종 분류·강제 제외를 적용하고, 일반 식당 우선 → 후순위 순서로 최대 8건을 유지한다. 같은 등급 내 거리순과 다른 카테고리의 8건 상한, 기존 반경·원천 보충·캐시 정책은 유지한다. 15건 밖의 식당까지 모두 탐색하지는 않는다.
- 식사 조회가 성공 반환됐지만 적격 후보가 없으면 블록의 `placeName/address/lat/lng=null`, `description="추천할 식당을 찾지 못했어요. 편집에서 장소를 추가해 보세요."`다. 외부 식사 조회가 예외로 종료됐으면 같은 장소 null 형식에 `description="식당 정보를 불러오지 못했어요."`를 사용한다. 어느 경우에도 전체 생성은 실패하지 않는다(NFR-3·결정-75).
- 성공 반환에는 한 원천이 실패해도 다른 원천의 결과로 성공한 부분 조회도 포함된다(결정-49). 그 결과가 모두 분류 미확인 등으로 제외되면 적격 0건 안내를 쓴다. 해당 문구와 내부 `sources=LIVE`는 모든 원천의 정상 응답을 뜻하지 않는다.
- 식당이 선택된 식사 블록의 `description`은 `방문 전 메뉴와 영업시간을 확인해 주세요.`다. 제목을 되풀이하거나 확인하지 않은 메뉴·소화·회복 효능 또는 이동 절감을 단정하지 않는다.
...
### 6-5 `GET /api/courses/loop` — 걷기 스팟을 진입점으로 하는 순환 경로 🔒 확정 (결정-68 · 2026-09-15)

> **왜 생겼나.** 출시 후 "러닝코스 탭인데 공원은 핀만 서 있고 경로가 없다" 는 피드백. 걷기 스팟은 카카오
> 장소 검색 결과라 좌표 하나뿐이고 경로 데이터가 없다(§4-3). 앱 안에서 경로를 만들 수 없으니(결정-42)
> **서버가 이미 가진 `generateOsmRoundTrip` 을 스팟 좌표로도 돌릴 수 있게 문을 하나 더 연다.**
> 목록·출발지는 그대로 두고 **사용자가 탭한 스팟 하나의 경로만** 만든다 — `near` 응답에 스팟마다 경로를
> 미리 붙이면 요청당 GraphHopper 를 최대 12번 돌려 첫 화면이 느려진다.

**출발지 의미는 바뀌지 않는다 🔒 — 스팟은 출발지가 아니라 경로의 진입점(entry point)이다.**
`near` 의 큐레이션 `ROUTE` 가 이미 그렇다: 출발지에서 `distanceM` 만큼 떨어진 진입점에서 경로가 시작하고,
저장 snapshot 의 `entryLat/entryLng` 도 출발지가 아니라 경로 시작점이다(§7-A). 이 API 는 그 진입점을
사용자가 탭한 스팟으로 두는 것뿐이다. 그래서 **출발지 SSOT 는 화면의 출발지(`lat/lng`) 하나**고,
`OriginState`·출발지 이력·복원 규칙(#269 · 결정-56)은 건드리지 않는다. 응답 `distanceM` 은 `near` 와
같은 뜻 — **출발지에서 경로 시작점까지** — 이라 서버가 출발지 좌표를 받아 계산한다.

`?lat=&lng=&entryLat=&entryLng=&targetKm=5&entryName=`
...
| `entryName` | 선택. 경로 이름에만 쓴다 — `near` 의 `PLACE.name`. 서버는 제어문자를 제거하고 앞뒤 공백을 제거한다. 정제 결과가 비면 값이 없는 것으로 처리해 §6-1 의 기본 이름(`출발지 주변 …`)을 쓴다. 50자를 초과하면 **Unicode 코드 포인트 기준 앞 50자**로 잘라 쓴다. `entryName` 때문에 `400 VALIDATION_FAILED` 를 반환하지 않는다 🔒확정(2026-09-16 서버 담당) |
...
- `name` — `entryName` 이 있으면 `{entryName} 주변 {routeKm 반올림}km {평지|완만} 러닝코스`, 없으면 §6-1 기존 규칙. 서버는 `entryName` 의 제어문자를 제거하고 앞뒤 공백을 제거한 뒤, 비면 없는 것으로 보고 50자를 넘으면 Unicode 코드 포인트 기준 앞 50자로 자른다. 그 외 검증·조회·캐시 키·GraphHopper 입력에는 쓰지 않는다. — 경로 생성의 핵심 입력이 아니라 화면에 붙이는 이름이고, 값의 출처도 서버 자신의 `near` 응답이라 이것 때문에 경로 생성을 400 으로 막으면 사용자가 복구할 방법이 없다(#353 리뷰 · 2026-09-16 서버 담당 확정). 저장 시 `courseName` snapshot 은 이 값이다(§7-A).
...
- `400 VALIDATION_FAILED` — 좌표 범위 밖 · `targetKm` 범위/단위 위반. `entryName` 때문에 400 을 반환하지 않는다 — 비거나 길면 위 규칙대로 정제한다.
...
**확정 근거** (2026-09-15 · 서버 담당 결정 · #353):
1. "스팟 = 진입점" 읽기로 출발지 충돌을 푼다. `near` 의 큐레이션 `ROUTE` 가 이미 화면 출발지와 경로 진입점을 `distanceM` 으로 분리하고 있어 일관된다.
2. 캐시는 **5분 유지.** 서버의 위치·외부 조회 공통 Caffeine TTL 과 같고, graph artifact 를 교체했을 때 이전 geometry 가 남는 시간도 5분으로 묶인다. 운영 부하 근거 없이 먼저 늘리지 않는다 — artifact 버전을 캐시 키에 넣거나 교체 시 evict 하는 계약이 생기면 그때 다시 본다.
3. 오류는 **`503 COURSE_SOURCES_UNAVAILABLE` 하나.** 비활성·readiness 미완료·호출 실패·timeout 모두 앱에서는 같은 요청을 다시 시도하는 상태이고 `near` 와도 같다. 원인은 공개 오류 코드를 늘리지 않고 서버 로그·운영 지표에서 구분한다.
...
| 7 러닝코스 출발지 주변 | 출발지 검색/프리셋 / 경로·장소 통합 목록 / 걷기 스팟 탭 → 순환 경로 | 4-4·(클라 상수) / 6-1 / 6-5 🔧초안 |
