...
비밀번호는 BCrypt strength 10 단방향 해시(NFR-9). 현재 서버 활성 버전은 `TOS=1.1`, `PRIVACY=1.2`, `MARKETING=1.1`이며 선택 동의를 포함한 세 행을 같은 `changedAt`에 append-only 저장하고 앱은 버전을 보내지 않는다(NFR-12, 결정-52(09-11 개정), 이슈 #111). `ageOver14` 전용 DB 컬럼이나 `AgreementType`은 두지 않는다. TOS 1.1 동의 시각·버전이 만 14세 이상 제한 확인 기록 역할을 한다(결정-58, 이슈 #228). 앱·서버 값은 같은 PR에서 함께 올렸고(#278), 운영 서버 전환은 출시 때 수행한다.
