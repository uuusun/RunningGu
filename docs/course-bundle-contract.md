...
`gainM=0`으로 보존할 수 있지만 출발지 주변 자동 경로 후보에서는 제외한다.

원본 GPX에 여러 `<trk>`가 있거나 한 트랙 안에 끊긴 조각이 있으면 500m를 초과하는 점 사이를
서로 다른 후보로 분리한다. 후보가 둘 이상이면 원천 표기 거리에 가장 가까운 조각을 코스 경로로
선택하되, `distanceKm` 값 자체는 선택한 GPX 좌표로 다시 계산한다. 원천 표기 거리로 실측값을
덮어쓰지 않는다. 이는 DMZ 8-1 코스의 5.82km 순간이동과 서해랑길 60 코스의 잘못된 트랙 선택을
막기 위한 생산자 규칙이다(이슈 #130).
...
KTO 메타 키가 없는 환경에서 승인된 261개 시드 ID와 공개 GPX만으로 초기 번들을 만들 수 있다.
이 산출물은 현재 프로세스의 KTO 전체 동기화 전 fallback이므로 모든 두루누비 코스를
`GPX_ONLY`로 기록한다. 서버가 전체 `courseList`를 성공적으로 받은 뒤 매칭한 항목만 런타임
snapshot에서 `API_GPX`와 `syncedAt`을 가진다.

261은 현재 두루누비 시드 기준 수치이지 영구 상수는 아니다. 다만 승인된 시드나 라이선스
검증 완료 원천을 명시적으로 변경하지 않은 실행에서 코스 수가 261 미만이면 새 파일을
승격하지 않는다.

번들은 자동 갱신하지 않는다. 데이터 담당자가 새 코스의 GPX·라이선스 검증을 마쳤거나 번들
스키마가 바뀌거나 출시·제출용 데이터를 동결할 때 수동으로 재생성하고, 생성 로그와 결정성
검증 결과를 확인한 뒤 별도 `data(course):` PR로 승격한다. KTO에 새 메타가 있다는 이유만으로
검증되지 않은 `API_ONLY` 경로를 번들에 추가하지 않는다.
