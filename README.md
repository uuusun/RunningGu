...
> 전국 마라톤 일정 통합 + 대회 전후 **여행 동선 자동 추천** + 출발지 기준 **러닝·산책 코스**를 갖춘 러닝 관광 **안드로이드 앱**
>
...
핵심 기능(M1~M4·B): 종목·상황별 맞춤 동선 추천 · 전국 마라톤 통합 캘린더 · 대회 인근 축제 · 러닝/산책 코스 · 찜·마이.
**기기 위치는 쓰지 않는다** — 출발지는 사용자가 검색·프리셋으로 직접 고른다(SPEC 결정-56).
...
> 참고: `reference-web/`은 **UX·화면흐름·로직 설계 참조**로만 쓴다. 앱의 초기 대회본(`races.json`)은 `android/app/src/main/assets/`에 두되, 온라인 데이터의 SSOT는 백엔드다. GPX 축약 번들은 P0 에 없다(P1 · SPEC 결정-68).
...
| **김민지** | **앱 코어** — 프로젝트 셋업·Retrofit/Room·공통 ProblemDetail/Enum/페이징 계약·카카오맵·두루누비+GPX 코스·UTF-8 데이터 CI |
